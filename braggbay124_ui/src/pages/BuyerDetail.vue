					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateBuyer()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Buyers = true">Add</base-button>
								              <modal :show.sync="modal2Buyers">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Buyer Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="BuyerId" type="text" placeholder="Enter BuyerId" v-model="buyerDetails.buyerId"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateBuyer() {
      ...
      const jsonData = JSON.stringify(this.buyerDetails);
      const res = await BuyerService.update(jsonData);
...
//        this.getAllBuyers();
    ...
    async getBuyerDetails() {
      try {
			let response = await BuyerService.get(this.buyerId);
			this.buyerDetails = response.data;

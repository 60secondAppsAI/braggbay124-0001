					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateBid()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Bids = true">Add</base-button>
								              <modal :show.sync="modal2Bids">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Bid Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="BidId" type="text" placeholder="Enter BidId" v-model="bidDetails.bidId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
...
export default {
  ...
  methods: {
    ...
    async updateBid() {
      ...
      const jsonData = JSON.stringify(this.bidDetails);
      const res = await BidService.update(jsonData);
...
//        this.getAllBids();
    ...
    async getBidDetails() {
      try {
			let response = await BidService.get(this.bidId);

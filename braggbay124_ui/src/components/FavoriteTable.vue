        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalFavorites = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFavorites">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Favorite</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FavoriteId" type="text" placeholder="Enter FavoriteId" v-model="favoriteToAdd.favoriteId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="favorites" :row-key="record => record.FavoriteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FavoritePictureView :favorites="favorites" />
          </div>
...
import FavoritePictureView from './FavoritePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'favoriteId',
          visible: true,
          scopedSlots: { customRender: 'favoriteId' },
          sorter: true
          //sorter: (a, b) => a.favoriteId - b.favoriteId,
          //sorter: (a, b) => a.favoriteId.localeCompare(b.favoriteId),
        },
      ],
      pagination: {
        current: 1,
      ...
      sortBy: 'favoriteId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFavoritesTable() {
      ...
      let favoritesTableData = [];
      for (let i = 0; i < this.favorites.length; i++) {
        favoritesTableData.push({
          id: i,
          favoriteId: this.favorites[i].favoriteId,
          year: this.favorites[i].year,
          date: this.favorites[i].date,
          competitionId: this.favorites[i].competitionId,
          favoriteId: this.favorites[i].favoriteId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-favorites',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFavoritesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.favoriteToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.favoriteToAdd);
      console.log(jsonData);
      
      const res = await FavoriteService.addFavorite(jsonData);

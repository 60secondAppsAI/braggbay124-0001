        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalInvoices = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInvoices">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Invoice</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="InvoiceId" type="text" placeholder="Enter InvoiceId" v-model="invoiceToAdd.invoiceId"></base-input>
  <base-input label="IssuedDate" type="text" placeholder="Enter IssuedDate" v-model="invoiceToAdd.issuedDate"></base-input>
  <base-input label="AmountDue" type="text" placeholder="Enter AmountDue" v-model="invoiceToAdd.amountDue"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="invoices" :row-key="record => record.InvoiceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InvoicePictureView :invoices="invoices" />
          </div>
...
import InvoicePictureView from './InvoicePictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'issuedDate',
          visible: true,
          scopedSlots: { customRender: 'issuedDate' },
          sorter: true
		...
		dataIndex: 'amountDue',
          visible: true,
          scopedSlots: { customRender: 'amountDue' },
          sorter: true
      ...
      sortBy: 'invoiceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInvoicesTable() {
      ...
      for (let i = 0; i < this.invoices.length; i++) {
        invoicesTableData.push({
          id: i,
          invoiceId: this.invoices[i].invoiceId,
          year: this.invoices[i].year,
          date: this.invoices[i].date,
          competitionId: this.invoices[i].competitionId,
          invoiceId: this.invoices[i].invoiceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-invoices',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInvoicesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.invoiceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.invoiceToAdd);
      console.log(jsonData);
      
      const res = await InvoiceService.addInvoice(jsonData);

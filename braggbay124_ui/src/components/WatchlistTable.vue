        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalWatchlists = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalWatchlists">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Watchlist</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="WatchlistId" type="text" placeholder="Enter WatchlistId" v-model="watchlistToAdd.watchlistId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="watchlists" :row-key="record => record.WatchlistId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <WatchlistPictureView :watchlists="watchlists" />
          </div>
...
import WatchlistPictureView from './WatchlistPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    WatchlistPictureView
  },

  data() {
    return {
      modalWatchlists: false,
        isTableView: true,

      columns: [
        {
          title: 'Watchlist Id',
		dataIndex: 'watchlistId',
          visible: true,
          scopedSlots: { customRender: 'watchlistId' },
          sorter: true
          //sorter: (a, b) => a.watchlistId - b.watchlistId,
          //sorter: (a, b) => a.watchlistId.localeCompare(b.watchlistId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} watchlists`,
      },
      ...
      sortBy: 'watchlistId',           // Column to sort by
  ...
  methods: {
    ...
    async renderWatchlistsTable() {
      ...
      let watchlistsTableData = [];
      for (let i = 0; i < this.watchlists.length; i++) {
        watchlistsTableData.push({
          id: i,
          watchlistId: this.watchlists[i].watchlistId,
          year: this.watchlists[i].year,
          date: this.watchlists[i].date,
          competitionId: this.watchlists[i].competitionId,
          watchlistId: this.watchlists[i].watchlistId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-watchlists',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    routingToFavoriteDetail(id) {
      this.$router.push({ name: 'FavoriteDetail', params: { favoriteId: id.toString() }})
    },
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForWatchlistsChanged', this.searchQuery);
		//this.renderWatchlistsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalWatchlists = false;

      const currentDate = new Date().getTime();
      this.watchlistToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.watchlistToAdd);
      console.log(jsonData);
      
      const res = await WatchlistService.addWatchlist(jsonData);
